...
cdef class OsContextBase(object):
    ...
    @mc
    def VertexPointer(self, np.ndarray[float, ndim=2] pointer not None): _glVertexPointer(pointer.shape[1], GL_FLOAT, 0, &pointer[0,0])
    @mc
    def VertexPointer0(self): _glVertexPointer(3, GL_FLOAT, 0, NULL)
    @mc
    def ColorPointer0(self): _glColorPointer(3, GL_FLOAT, 0, NULL)
    @mc
    def ColorPointerd(self, np.ndarray[float, ndim=2] pointer not None): _glColorPointer(3, GL_FLOAT, 0, &pointer[0,0])
    @mc
    ...
    def getDepthCloud(self, artificial_depth=None, camera_space=False):
        ...
        # mtx = np.zeros((4,4), order='F', dtype=np.float64)
        ...
        winX = np.arange(self.w*self.h, dtype=np.float64) % self.w
        winY = np.arange(self.w*self.h, dtype=np.float64) // self.w
        ...
        P = np.zeros((4,4), order='F', dtype=np.float64)
        _glGetFloatv(GL_PROJECTION_MATRIX, <GLfloat*>(&P[0,0]))
        
        cdef np.ndarray[float, ndim=2] M
        M = np.asarray(np.eye(4), dtype=np.float64, order='F')
    ...
    def getDepthCloud_old(self, depth=None):
        cdef int viewport[4]
        cdef float modelview[16]
        cdef float projection[16]
        cdef float winX, winY, winZ
        cdef float winX2, winY2, winZ2
        cdef float posX, posY, posZ
        cdef float camposX, camposY, camposZ
        cdef np.ndarray[np.float64_t, ndim=2] modelview_eye = np.eye(4)
        #cdef double modelview_eye[16]
         
        _glGetFloatv( GL_MODELVIEW_MATRIX, modelview );
        _glGetFloatv( GL_PROJECTION_MATRIX, projection );
        _glGetIntegerv( GL_VIEWPORT, viewport );
        ...
        if depth is None:
            ...
            proj = np.zeros((4,4), order='F', dtype=np.float64)
